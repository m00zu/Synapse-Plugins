...
class Node3DViewerWidget(NodeBaseWidget):
    ...
    def __init__(self, parent=None, enable_bridge=False):
        ...
        try:
            ...
            lbl = QtWidgets.QLabel(
                "3D Viewer requires QtWebEngine.\n"
                "Run: pip install PySide6-Addons\n\n"
                "Click the node to open in browser instead.")
            lbl.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
            lbl.setStyleSheet("color: #aaa; font-size: 9pt;")
            lbl.setFixedSize(self._DISPLAY_W, self._DISPLAY_H)
    ...
    def set_value(self, data):
        ...
        """
        if not self._has_webengine or not isinstance(data, dict):
            return
        if self._page_ready:
